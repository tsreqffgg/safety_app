...
# --- الواجهة الرئيسية ---
st.title("🛡️ SAFETY 360")
st.markdown("##### مساعدك الفوري والقانوني للسلامة المهنية وإدارة الأزمات (الاردن ودولي)")
st.write("")

# نافذة البحث الرئيسية
user_query = st.text_input("🔍 ابحث عن أي تشريع، خطر، طوارئ، حفريات، كيميائي، أو إجراء سلامة:", placeholder="اكتب كلمة البحث هنا (مثلاً: ارتفاع، حريق، LOTO)...")
    ...
    with st.spinner("⏳ جارٍ البحث في قاعدة المعرفة المعتمدة..."):
        if not df.empty:
            ...
            if not results.empty:
                ...
                st.warning("⚠️ لم يتم العثور على مطابقة دقيقة، جرب كلمات أخرى مثل: ( طوارئ، ارتفاع، حريق، كهرباء).")
        else:
